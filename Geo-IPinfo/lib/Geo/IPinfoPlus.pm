...
sub new {
    ...
    $header->header( 'User-Agent'   => 'IPinfoClient/Perl/3.4.0' );
...
sub _get_info {
    ...
    return $info if eval { $info->isa('Geo::DetailsPlus') };

    return defined $info ? Geo::DetailsPlus->new( $info ) : undef;
}
